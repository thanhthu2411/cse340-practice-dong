...
const flashMiddleware = (req, res, next) => {
    // Sometimes the session is not fully saved yet before the redirect response is sent.
    // -> If the session changed, save it first — then redirect.
    let sessionNeedsSave = false;

    // Override res.redirect to save session before redirecting
    const originalRedirect = res.redirect.bind(res);
                    // collect all arguments passed into res.redirect into an array called 'args'
    res.redirect = (...args) => {
        if (sessionNeedsSave && req.session) {
            // Save session before redirecting
            req.session.save(() => {
                // .apply() allows a function to run with attached arguments, like apply these arguments when the function runs
